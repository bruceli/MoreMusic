...
- (NSMutableData *)postBody
{
    ...
    {
        //TEST AREA
        //NSString* testString =[WBRequest stringFromDictionary:params];
        //NSLog(@"%@" ,testString);
        //THE END OF AREA
...
+ (id)parseJSONData:(NSData *)data error:(NSError **)error
{
	...
	if (parseError)
    {
        if (error != nil)
        {
           // *error = [self errorWithCode:kWBErrorCodeSDK userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"%d", kWBSDKErrorCodeParseError] forKey:kWBSDKErrorCodeKey]];
            *error = [NSError errorWithDomain:kWBSDKErrorDomain code:kWBErrorCodeSDK userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"%d", kWBSDKErrorCodeParseError] forKey:kWBSDKErrorCodeKey]];
        }
	}
	...
	if ([result isKindOfClass:[NSDictionary class]])
	{
		if ([result objectForKey:@"error_code"] != nil && [[result objectForKey:@"error_code"] intValue] != 200)
		{
			if (error != nil) 
			{
                *error = [NSError errorWithDomain:kWBSDKErrorDomain code:kWBErrorCodeInterface userInfo:result];
			}
		}
	}
