...
- (WBRequest*)getRequestWithMethodName:(NSString *)methodName
                       httpMethod:(NSString *)httpMethod
                           params:(NSDictionary *)params
                     postDataType:(WBRequestPostDataType)postDataType
                 httpHeaderFields:(NSDictionary *)httpHeaderFields
{
    ...
    WBRequest* req = [WBRequest requestWithAccessToken:accessToken
                                                 url:[NSString stringWithFormat:@"%@%@", kWBSDKAPIDomain, methodName]
                                          httpMethod:httpMethod
                                              params:params
                                        postDataType:postDataType
...
-(void)favoritMessageWithWeiboID:(NSNumber*)weiboID withStatus:(BOOL)favoStatus
{
    ...
    if (!favoStatus) {  // save as favorit
        ...
        [self loadRequestWithMethodName:@"favorites/destroy.json"
                             httpMethod:@"POST"
                                 params:params
                           postDataType:kWBRequestPostDataTypeNormal
                       httpHeaderFields:nil];
...
{    
    ...
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithCapacity:1];
    [params setObject:(idString ? idString : @"") forKey:@"id"];
    WBRequest *rq = [self getRequestWithMethodName:@"comments/show.json"
                           httpMethod:@"GET"
                               params:params
                         postDataType:kWBRequestPostDataTypeNone
                     httpHeaderFields:nil];
    if (inDelegate) {
        rq.delegate = inDelegate;
    }
...
{
    //    NSString* idString = @"3448447732599672";
    
    NSString* idString = [weiboID stringValue];
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithCapacity:1];
    [params setObject:(idString ? idString : @"") forKey:@"id"];
    WBRequest *rq = [self getRequestWithMethodName:@"statuses/repost_timeline.json"
                                        httpMethod:@"GET"
                                            params:params
                                      postDataType:kWBRequestPostDataTypeNone
                                  httpHeaderFields:nil];
